...
static uint8_t BlockBuffer[SECTOR_SIZE_BYTES];
...
void VirtualFAT_ReadBlock(const uint16_t BlockNumber)
{
	...
	case DISK_BLOCK_FATBlock2:
		...
		UpdateFAT12ClusterChain(FLASHFileStartCluster, (const uint8_t)FILE_CLUSTERS(FLASH_FILE_SIZE_BYTES));
...
void VirtualFATWrite(uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
...
void VirtualFATRead(uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
	for (uint16_t i = 0; i < blk_len; i++)
	{
